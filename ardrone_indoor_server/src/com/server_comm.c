...
int server_listen_drone(int lg_mesg_emis)
{
...
int server_send_drone(char * dest, char *signal)
{
   ...
   if ((sock = socket(AF_INET, SOCK_DGRAM, 0))==-1)
   {
           printf("\n   Server communication thread : sending : error during the socket creation\n");
           error_type = ERROR_TYPE_SOCKET_CREATION;
   }
   ...
   printf("\n     Server communication thread : sending : %d bytes\n\n", sizeof(char));
