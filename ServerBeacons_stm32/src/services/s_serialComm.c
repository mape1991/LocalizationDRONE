...
void s_serialComm_it_function(void)
{
	...
	// get received data
	data = Receive_Data_USART(USART1);
...
char s_serialComm_initialization(void (*ptrFunction) (unsigned int))
{
	...
	if ((code_Erreur = Init_USART (USART1, BAUD_RATE, EVEN, STOP_BIT_MODE3, WORD_LENGTH_8BITS)) != 0 )
	{
...
{ 		
	...
	if ( (code_Erreur = Send_Data_USART(USART1, (unsigned int) c)) != 0 )
	{
